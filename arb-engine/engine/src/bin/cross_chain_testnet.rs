...
async fn main() -> Result<()> {
    ...
    let evm_provider = match ProviderBuilder::new().on_ws(evm_conn).await {
        ...
        Err(e) => {
            warn!("Arbitrum Sepolia connection failed: {}. Falling back to simulated mode.", e);
            None
        }
    };
    ...
    match solana_client.get_version().await {
        ...
        Err(e) => warn!("Solana Devnet connection failed: {}. Continuing with simulated execution.", e),
    }

    // 3. Set up Solana Signer
    // Generate a temporary execution keypair for simulation, or load from PRIVATE_KEY/KEYPAIR path
    let solana_keypair = Keypair::new();
    info!("🔑 Loaded Solana execution Keypair: {}", solana_keypair.pubkey());
    ...
    info!("✓ Jito Tip Instruction built successfully to Tip Account: {}", tip_pubkey);
    ...
    info!("✓ Solana Arbitrage Instruction compiled. Data size: {} bytes", arb_ix.data.len());
    ...
    info!("✓ Transaction serialized successfully! Size: {} bytes", serialized_tx.len());
        ...
        Ok(resp) => {
            if resp.status().is_success() {
                ...
                if let Some(err) = json.get("error") {
                    ...
                    info!("✅ Jito Bundle successfully processed! Response: {:?}", json);
                }
            } else {
