...
pub fn get_amount_in(pool: &Pool, amount_out: U256, zero_for_one: bool) -> Result<U256> {
    ...
    if amount_out >= reserve_out {
        bail!("amount_out ({}) exceeds reserve_out ({})", amount_out, reserve_out);
    }
...
mod tests {
    ...
    fn make_pool(reserve_a: u128, reserve_b: u128, fee_bps: u32) -> Pool {
        Pool {
            ...
            token_a: Token { address: "0xWETH".into(), symbol: "WETH".into(), decimals: 18 },
            token_b: Token { address: "0xUSDC".into(), symbol: "USDC".into(), decimals: 6 },
    ...
    fn test_amount_out_basic() {
        ...
        assert!(out_decimal > 2.9 && out_decimal < 3.0, "out = {}", out_decimal);
    }
