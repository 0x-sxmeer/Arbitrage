...
fn get_amount1_delta(sqrt_lower: U256, sqrt_upper: U256, liquidity: u128) -> Result<U256> {
...
fn get_amount0_delta(sqrt_lower: U256, sqrt_upper: U256, liquidity: u128) -> Result<U256> {
    if sqrt_lower > sqrt_upper {
...
pub fn tick_to_sqrt_price_x96(tick: i32) -> Result<U256> {
    ...
    if !scaled.is_finite() || scaled < 0.0 {
        bail!(
            "tick_to_sqrt_price_x96: non-finite result for tick {}",
            tick
        );
    }
...
mod tests {
    ...
    fn test_higher_fee_produces_less_output() {
        ...
        assert!(
            out_low >= out_high,
            "Lower fee should produce more or equal output: low={:?} high={:?}",
            out_low,
            out_high
        );
    }
    ...
    fn test_price_impact_small_trade() {
        ...
        assert!(
            impact < 100,
            "Expected impact < 1% for tiny trade, got {} bps",
            impact
        );
    }
    ...
    fn test_simulate_swap_updates_sqrt_price() {
        ...
        assert!(
            updated < original,
            "sqrt price should decrease after selling token0"
        );
    }
    ...
    fn test_simulate_swap_noop_on_v2_pool() {
        ...
        assert_eq!(
            pool.state.sqrt_price_x96, original,
            "V2 pool should not be modified"
        );
    }
