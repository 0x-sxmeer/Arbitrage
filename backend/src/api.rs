...
pub async fn start_api_server(
    ...
    let app = Router::new()
        ...
        .route("/api/cex-dex", get(get_cex_dex))
        .route("/api/liquidations", get(get_liquidations))
        .route("/api/cross-chain", get(get_cross_chain))
        .route("/api/profit-history", get(get_profit_history))
        .layer(cors)
...
async fn get_cex_dex() -> Json<serde_json::Value> {
    Json(serde_json::json!({
        "opportunities": [],
        "prices": {}
    }))
...
async fn get_liquidations() -> Json<Vec<serde_json::Value>> {
    Json(vec![])
}

async fn get_cross_chain() -> Json<serde_json::Value> {
    Json(serde_json::json!({
        "opportunities": [],
        "prices": {}
    }))
...
async fn get_profit_history() -> Json<Vec<serde_json::Value>> {
    let now = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_millis();
    Json(vec![serde_json::json!({"t": now, "profit": 0})])
}
