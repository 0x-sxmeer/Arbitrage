    ...
    match chain {
        ChainId::Base => vec![
            PoolConfig {
                ...
                pool_address: "0xd0b53D9277642d899DF5C87A3966A349A798F224".into(),
                token0_is_token: false,
                fee_bps: 5,
                is_v3: true,
            },
            PoolConfig {
                token_symbol: "WBTC".into(),
                pool_address: "0xFd36cE0A6cCDE9B3F5c9b0e17Adb0fBE55CAadCb".into(),
                token0_is_token: true,
                fee_bps: 30,
                is_v3: true,
            },
            PoolConfig {
                token_symbol: "ETH".into(),
                pool_address: "0x7c3B2a83A6cF0b8Ca2c0c8A5A47af5b7F6B2dAB2".into(),
                token0_is_token: false,
                fee_bps: 5,
                is_v3: false,
            },
        ...
        ChainId::Optimism => vec![
            PoolConfig {
                token_symbol: "ETH".into(),
                pool_address: "0x85149247691df622eaF1a8Bd0CaFd40BC45154a".into(),
                token0_is_token: false,
                fee_bps: 5,
                is_v3: true,
            },
            PoolConfig {
                token_symbol: "WBTC".into(),
                pool_address: "0x73B14a78a0D396C521f954532d43fd5fFe7277b".into(),
                token0_is_token: true,
                fee_bps: 30,
                is_v3: true,
            },
        ...
        ChainId::Arbitrum => vec![
            PoolConfig {
                token_symbol: "ETH".into(),
                pool_address: "0xC31E54c7a869B9FcBEcc14363CF510d1c41fa443".into(),
                token0_is_token: false,
                fee_bps: 5,
                is_v3: true,
            },
            PoolConfig {
                token_symbol: "WBTC".into(),
                pool_address: "0x2f5e87C9312fa29aed5c179E456625D79015299c".into(),
                token0_is_token: true,
                fee_bps: 30,
                is_v3: true,
            },
            PoolConfig {
                token_symbol: "ARB".into(),
                pool_address: "0x81C48D31365e6B526f6BBadC5c9aaFd822134863".into(),
                token0_is_token: true,
                fee_bps: 30,
                is_v3: true,
            },
...
impl ChainMonitor {
    pub fn new(chain: ChainId, rpc_url: String, matrix: PriceMatrix, poll_ms: u64) -> Self {
        Self {
    ...
    async fn poll_all_pools(&self) -> Result<()> {
        ...
        for pool in &self.pools {
            ...
            let should_update = existing
                .map(|p| ((price_usd - p.price_usd) / p.price_usd).abs() > 0.00001)
                .unwrap_or(true);
    ...
    async fn fetch_pool_price(&self, _pool: &PoolConfig) -> Option<f64> {
