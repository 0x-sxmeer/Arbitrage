...
use crate::arb::router::{
    find_arbitrage_cycles, reset_changed_tokens, LiquidityGraph, RouterConfig,
};
use crate::chains::evm::EvmAdapter;
use crate::db::postgres::PostgresStore;
use crate::db::redis::RedisCache;
use crate::mempool::calldata_decoder::{classify_router, decode_swap, DexVersion};
...
pub struct MempoolListener {
    ...
    /// BackrunDetector — plugged in from main.rs after construction
...
impl MempoolListener {
    pub fn new(
        ...
        Self {
            ...
            backrun_detector: None,
        }
    }

    /// Attach a BackrunDetector — call before spawning run().
    pub fn with_backrun_detector(
        mut self,
        detector: Arc<crate::liquidations::backrun::BackrunDetector>,
    ) -> Self {
        self.backrun_detector = Some(detector);
        self
    }
    ...
    async fn run_solana_stream(&self) -> Result<()> {
        ...
        loop {
                        ...
                        for pool in solana_pools {
                            ...
                            let h = tokio::spawn(async move {
                                if let Ok(pubkey) = solana_sdk::pubkey::Pubkey::from_str(&pool.id) {
                                    if let Ok((mut sub, _)) =
                                        c.account_subscribe(&pubkey, Some(cfg)).await
                                    {
            ...
            reconnect_delay =
                ((reconnect_delay as f64 * BACKOFF_MULTIPLIER) as u64).min(MAX_RECONNECT_MS);
        }
    }
    ...
    async fn connect_and_stream(&self) -> Result<()> {
        ...
        tokio::spawn(async move {
            loop {
                ...
                match tokio::time::timeout(
                    ...
                    provider_check.get_block_number(),
                )
                .await
                {
                    Ok(Ok(_)) => {}
        ...
        if is_l2 {
            info!(
                "⛓ Real-time Block Execution Pipeline Active for Network: {:?}",
                active_chain
            );
            let sub = provider
                .subscribe_blocks()
            ...
            tokio::spawn(async move {
                ...
                if let Ok(provider_l2) = provider_l2 {
                        ...
                        tokio::spawn(async move {
                            while let Some(log) = log_stream.next().await {
                                ...
                                if let Some(ep) = g.get_pool(&pool_address) {
                                    ...
                                    {
                                        if let Ok(sync) =
                                            IUniswapV2Pair::Sync::decode_log(&log.inner, true)
                                        {
                                            up.state.reserve_a = crate::pool::U256::from_str_radix(
                                                &sync.reserve0.to_string(),
                                                10,
                                            )
                                            .unwrap_or_default();
                                            up.state.reserve_b = crate::pool::U256::from_str_radix(
                                                &sync.reserve1.to_string(),
                                                10,
                                            )
                                            .unwrap_or_default();
                                            ...
                                            g.upsert_pool(up);
                                            tracing::debug!(
                                                "Updated pool {} reserves: A: {}, B: {}",
                                                pool_address,
                                                sync.reserve0,
                                                sync.reserve1
                                            );
                                        }
                                    } else if log.topics().first()
                                        == Some(&IUniswapV3PoolEvents::Swap::SIGNATURE_HASH)
                                    {
                                        if let Ok(swap) =
                                            IUniswapV3PoolEvents::Swap::decode_log(&log.inner, true)
                                        {
                                            up.state.sqrt_price_x96 = Some(
                                                crate::pool::U256::from_str_radix(
                                                    &swap.sqrtPriceX96.to_string(),
                                                    10,
                                                )
                                                .unwrap_or_default(),
                                            );
                                            up.state.liquidity = Some(
                                                swap.liquidity
                                                    .to_string()
                                                    .parse::<u128>()
                                                    .unwrap_or_default(),
                    ...
                    while let Some(block) = stream.next().await {
                        ...
                        for chunk in pools.chunks(15) {
                            // Expanded chunk size for rapid lookup
                            ...
                            .await
                        {
                            Ok(Some(block_with_txs)) => {
                                let mut sent = 0;
                                if let alloy::rpc::types::BlockTransactions::Full(txs) =
                                    block_with_txs.transactions
                                {
                                    tx_count_in_block = txs.len();
                                    for tx in txs {
                        ...
                        if tx_count_in_block > 50 {
                            tracing::info!("Block {} has {} txs (>50). Skipping Phase 1 pathfinder to save compute.", block_number, tx_count_in_block);
                        } else {
                            let start_tokens = [
                            ...
                            let live_gas = f64::from_bits(
                                live_gas_gwei_arc.load(std::sync::atomic::Ordering::Relaxed),
                            );
                            config.gas_price_gwei = if live_gas > 0.001 {
                                live_gas
                            } else {
                                router_config.gas_price_gwei
                            };
                            ...
                            let opportunities = {
                                ...
                                opps.sort_by(|a, b| {
                                    b.net_expected_value.cmp(&a.net_expected_value)
                                });
                                opps
                            };
                            ...
                            for opp in opportunities {
                                ...
                                metrics
                                    .add_recent_opportunity(
                                        &opp,
                                        router_config.eth_price_usd,
                                        router_config.btc_price_usd,
                                    )
                                    .await;
                                ...
                                match evm_adapter.simulate_arbitrage(&opp).await {
                                    Ok(()) => {
                                        info!(
                                            "🔥 Arbitrage Loop Found! Route: {} Net Value: {} Wei",
    ...
    fn make_worker_ctx(&self) -> WorkerCtx {
        WorkerCtx {
            ...
            backrun_detector: self.backrun_detector.clone(),
        }
    }
}
...
struct WorkerCtx {
    ...
    metrics: Arc<EngineMetrics>,
    execute_enabled: bool,
    /// BackrunDetector — None if BACKRUN_ENABLED=false
    backrun_detector: Option<Arc<crate::liquidations::backrun::BackrunDetector>>,
}
...
impl WorkerCtx {
    async fn process_payload(&self, payload: RawTxPayload) {
        ...
        let amount_in = crate::pool::U256::from_str_radix(&decoded.amount_in.to_string(), 10)
            .unwrap_or_default();
        ...
        // Phase 3 Backrunning Trigger: Check large swap thresholds
        ...
        if token_in == weth && amount_in >= crate::pool::U256::from(66_000_000_000_000_000_000u128)
        {
            is_large_swap = true;
        }
        // > $200k @ $3000
        else if token_in == usdc && amount_in >= crate::pool::U256::from(200_000_000_000u64) {
            is_large_swap = true;
        }
        // > $200k
        else if token_in == cbbtc && amount_in >= crate::pool::U256::from(105_000_000u64) {
            is_large_swap = true;
        }
        // > $100k @ $95000
        else if token_in == aero
            && amount_in >= crate::pool::U256::from(33_000_000_000_000_000_000_000u128)
        {
            is_large_swap = true;
        } // > $50k @ $1.5

        if is_large_swap && self.router_config.backrun_enabled {
            // Note: backrun_enabled config passed down? Wait, router_config doesn't have it. Let's just run it if we're evaluating.
            tracing::info!("🐋 Phase 3 Trigger: Large swap detected (token_in: {}). Running backrun pathfinder.", token_in);
            let start_tokens = [weth, usdc, cbbtc];
            let opportunities = {
                let graph = self.graph.read().await;
                let mut opps = Vec::new();
                for st in start_tokens {
                    opps.extend(graph.find_opportunities(st, &self.router_config));
                }
                opps.extend(find_arbitrage_cycles(&graph, &self.router_config));
            ...
            for opp in opportunities {
                ...
                if opp.net_expected_value > 0i128 {
                    tracing::info!(
                        "🔥 Backrun Arb Found! Route: {} Net Value: {} Wei",
                        opp.route_description(),
                        opp.net_expected_value
                    );
                }
        ...
        // BackrunDetector evaluation (profit-aware, whale-score-weighted)
        if let Some(ref detector) = self.backrun_detector {
            use crate::liquidations::backrun::PendingSwap;
            
            // Build the updated PendingSwap struct
            let pending = PendingSwap {
                tx_hash:      payload.tx_hash.clone(),
                from:         String::new(), // omitted for brevity
                router:       payload.to_addr.clone(),
                token_in:     token_in.clone(),
                token_out:    token_out.clone(),
                amount_in_usd: 100_000.0, // placeholder to pass min_size_usd check
                pool_address: String::new(),
                gas_price:    (payload.gas_price_gwei * 1e9) as u128,
                chain_id:     8453, // Base
            };
            
            if let Some(opp) = detector.evaluate(&pending).await {
                tracing::info!(
                    "🎯 BackrunDetector | impact={:.1}bps profit=${:.2} target={}",
                    opp.impact_bps, opp.exp_profit_usd, opp.target_tx
                );
                if self.execute_enabled {
                    tracing::info!("  [BACKRUN] bundle would be submitted for target_tx={}", opp.target_tx);
                }
            }
        }
    } // end process_payload
            ...
            None => {
                let graph_pool = {
                    ...
                    graph
                        .get_all_pools()
                        .find(|(_, p)| {
                            ((p.token_a.address.to_lowercase() == token_in
                                && p.token_b.address.to_lowercase() == token_out)
                                || (p.token_a.address.to_lowercase() == token_out
                                    && p.token_b.address.to_lowercase() == token_in))
                                && p.fee_bps == fee_bps
                        })
                        .map(|(_, p)| (**p).clone())
                };
                ...
                if let Some(ref adapter) = self.evm_adapter {
                    let base_pool = if let Some(p) = graph_pool {
                        ...
                        if let Ok(real_address) = adapter
                            .query_pool_address(token_in, token_out, fee_bps, dex_version)
                            .await
                        {
                            p.id = real_address;
                            p.pool_type = match dex_version {
                                DexVersion::UniswapV3 | DexVersion::AerodromeV3 => {
                            ...
                            p.dex = match dex_version {
                                ...
                                | DexVersion::BaseSwap => crate::pool::DexProtocol::UniswapV2,
                                DexVersion::AerodromeV3 | DexVersion::UniswapV3 => {
                                    crate::pool::DexProtocol::UniswapV3
                                }
                    ...
                    match adapter.fetch_pool_state(&base_pool).await {
                        Ok(state) => {
                            ...
                            if let Ok(json) = serde_json::to_string(&p) {
                                let _ = self
                                    .redis_cache
                                    .set_raw(&pool_cache_key, &json, POOL_CACHE_TTL_SECS)
                                    .await;
                            }
                            p
                        }
...
fn build_placeholder_pool(token_in: &str, token_out: &str, fee_bps: u32, chain: ChainId) -> Pool {
    ...
    Pool {
        ...
        token_a: Token {
            ...
            decimals: crate::pool::get_token_decimals(&addr_a),
        },
        token_b: Token {
            address: addr_b.clone(),
            symbol: "TOK_B".to_string(),
            decimals: crate::pool::get_token_decimals(&addr_b),
        },
