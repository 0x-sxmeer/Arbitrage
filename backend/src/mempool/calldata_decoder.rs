...
pub fn decode_swap(input_data: &[u8], to_addr: &str) -> Option<DecodedSwap> {
    ...
    match dex {
        ...
        | DexVersion::BaseSwap => decode_v2_family(input_data, sel, dex),

        DexVersion::UniswapV3
        | DexVersion::PancakeSwapV3
        | DexVersion::SushiSwapV3
        | DexVersion::AerodromeV3 => decode_v3_family(input_data, sel, dex),

        DexVersion::UniversalRouter => decode_universal_router(input_data),

        DexVersion::RaydiumAmm | DexVersion::OrcaWhirlpool | DexVersion::OsmosisCosmos => None,

        DexVersion::Unknown => decode_v3_family(input_data, sel, DexVersion::Unknown)
...
fn decode_v2_family(input_data: &[u8], sel: [u8; 4], dex: DexVersion) -> Option<DecodedSwap> {
    ...
    if dex == DexVersion::AerodromeV2 {
        if let Ok(call) =
            IAerodromeV2Router::swapExactTokensForTokensCall::abi_decode(input_data, true)
        {
        ...
        if let Ok(call) =
            IAerodromeV2Router::swapExactETHForTokensCall::abi_decode(input_data, true)
        {
            if !call.routes.is_empty() {
                let mut path = Vec::new();
                path.push(call.routes[0].from);
        ...
        if let Ok(call) =
            IAerodromeV2Router::swapExactTokensForETHCall::abi_decode(input_data, true)
        {
            if !call.routes.is_empty() {
                let mut path = Vec::new();
                path.push(call.routes[0].from);
    ...
    match sel {
        SEL_V2_EXACT_IN => {
            let call = IUniswapV2Router::swapExactTokensForTokensCall::abi_decode(input_data, true)
                .ok()?;
            debug!(
                "Decoded V2 swapExactTokensForTokens: {} hops",
                call.path.len()
            );
        ...
        SEL_V2_EXACT_OUT => {
            let call = IUniswapV2Router::swapTokensForExactTokensCall::abi_decode(input_data, true)
                .ok()?;
            debug!(
                "Decoded V2 swapTokensForExactTokens: {} hops",
                call.path.len()
            );
        ...
        SEL_V2_ETH_EXACT_IN => {
            let call =
                IUniswapV2Router::swapExactETHForTokensCall::abi_decode(input_data, true).ok()?;
        ...
        SEL_V2_TOKENS_FOR_EXACT_ETH => {
            let call =
                IUniswapV2Router::swapTokensForExactETHCall::abi_decode(input_data, true).ok()?;
            debug!(
                "Decoded V2 swapTokensForExactETH (amountInMax = {})",
                call.amountInMax
            );
        ...
        SEL_V2_EXACT_TOKENS_FOR_ETH => {
            let call =
                IUniswapV2Router::swapExactTokensForETHCall::abi_decode(input_data, true).ok()?;
        ...
        SEL_V2_ETH_FOR_EXACT => {
            let call =
                IUniswapV2Router::swapETHForExactTokensCall::abi_decode(input_data, true).ok()?;
...
fn decode_v3_family(input_data: &[u8], sel: [u8; 4], dex: DexVersion) -> Option<DecodedSwap> {
    match sel {
        ...
        SEL_V3_EXACT_OUTPUT_SINGLE => {
            let call =
                IUniswapV3Router::exactOutputSingleCall::abi_decode(input_data, true).ok()?;
...
fn decode_universal_router(input_data: &[u8]) -> Option<DecodedSwap> {
    if input_data.len() < 47 {
        return None;
    } // 4 bytes selector + at least 43 bytes path

    let data = &input_data[4..];

    for offset in 0..=data.len().saturating_sub(43) {
        let fee_raw =
            u32::from_be_bytes([0, data[offset + 20], data[offset + 21], data[offset + 22]]);
        ...
        if token_in.is_zero() || token_out.is_zero() || token_in == token_out {
            continue;
        }
        if token_in[..8].iter().all(|&x| x == 0) || token_out[..8].iter().all(|&x| x == 0) {
            continue;
        }
...
pub fn classify_router(addr: &str) -> DexVersion {
    match addr {
        ...
        RouterRegistry::UNISWAP_V3_ROUTER_V1 | RouterRegistry::UNISWAP_V3_ROUTER_V2 => {
            DexVersion::UniswapV3
        }
        RouterRegistry::UNISWAP_UNIVERSAL_ETH | RouterRegistry::UNISWAP_UNIVERSAL_OLD => {
        ...
        RouterRegistry::PANCAKE_V2_ROUTER | RouterRegistry::PANCAKE_V2_ROUTER_ETH => {
            DexVersion::PancakeSwapV2
        }
...
mod tests {
    ...
    fn test_classifier_known_routers() {
        assert_eq!(
            classify_router(RouterRegistry::UNISWAP_V2_ROUTER),
            DexVersion::UniswapV2
        );
        assert_eq!(
            classify_router(RouterRegistry::UNISWAP_V3_ROUTER_V1),
            DexVersion::UniswapV3
        );
        assert_eq!(
            classify_router(RouterRegistry::UNISWAP_V3_ROUTER_BASE),
            DexVersion::UniswapV3
        );
        assert_eq!(
            classify_router(RouterRegistry::UNISWAP_UNIVERSAL_ETH),
            DexVersion::UniversalRouter
        );
        assert_eq!(
            classify_router(RouterRegistry::AERODROME_V2_ROUTER),
            DexVersion::AerodromeV2
        );
        assert_eq!(
            classify_router(RouterRegistry::AERODROME_UNIVERSAL),
            DexVersion::AerodromeV3
        );
        assert_eq!(
            classify_router(RouterRegistry::BASESWAP_ROUTER),
            DexVersion::AerodromeV2
        );
        assert_eq!(
            classify_router(RouterRegistry::SUSHISWAP_V2_ROUTER),
            DexVersion::SushiSwapV2
        );
        assert_eq!(
            classify_router(RouterRegistry::PANCAKE_V2_ROUTER),
            DexVersion::PancakeSwapV2
        );
    ...
    fn test_base_routers_are_known() {
        ...
        assert!(is_known_dex_router(
            RouterRegistry::SUSHISWAP_V3_ROUTER_BASE
        ));
    }
