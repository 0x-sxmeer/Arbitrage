...
impl SolanaAdapter {
        ...
        match pool_type {
            PoolType::ConstantProduct | PoolType::StableSwap => {
                ...
                let reserve_a = u64::from_le_bytes(
                    data[RAYDIUM_COIN_AMOUNT_OFFSET..RAYDIUM_COIN_AMOUNT_OFFSET + 8].try_into()?,
                );
                let reserve_b = u64::from_le_bytes(
                    data[RAYDIUM_PC_AMOUNT_OFFSET..RAYDIUM_PC_AMOUNT_OFFSET + 8].try_into()?,
                );
            ...
            PoolType::ConcentratedLiquidity => {
                ...
                let tick =
                    i32::from_le_bytes(data[ORCA_TICK_OFFSET..ORCA_TICK_OFFSET + 4].try_into()?);
    ...
    pub async fn send_jito_bundle(
        ...
        info!(
            "Submitting bundle of {} transactions to Jito Block Engine...",
            transactions.len()
        );
        ...
        let response = client.post(jito_url).json(&payload).send().await?;
