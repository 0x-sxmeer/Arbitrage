...
impl RedisCache {
    ...
    pub async fn set_pool(&self, pool: &Pool) -> Result<()> {
        ...
        let json = serde_json::to_string(pool).context("Failed to serialize pool")?;
        ...
        if let Some(ref conn) = self.conn {
            ...
            self.fallback_ttl.insert(
                key.clone(),
                std::time::Instant::now() + std::time::Duration::from_secs(POOL_STATE_TTL_SECS),
            );
        }
    ...
    pub async fn set_gas_price_gwei(&self, gwei: f64) -> Result<()> {
        if let Some(ref conn) = self.conn {
            ...
            self.fallback
                .insert("gas_price:gwei".to_string(), gwei.to_string());
            self.fallback_ttl.insert(
                "gas_price:gwei".to_string(),
                std::time::Instant::now() + std::time::Duration::from_secs(GAS_PRICE_TTL_SECS),
            );
        }
        Ok(())
    }
    ...
    pub async fn get_gas_price_gwei(&self) -> f64 {
        ...
        result.and_then(|s| s.parse::<f64>().ok()).unwrap_or(20.0)
    }
    ...
    pub async fn set_raw(&self, key: &str, value: &str, ttl_secs: usize) -> Result<()> {
        if let Some(ref conn) = self.conn {
            ...
            self.fallback_ttl.insert(
                key.to_string(),
                std::time::Instant::now() + std::time::Duration::from_secs(ttl_secs as u64),
            );
        }
        Ok(())
    }
...
mod tests {
    ...
    fn test_pool_key_solana() {
        ...
        assert_eq!(
            key,
            "pool:solana:8sLbNZoA1cfnvMJLPfp98ZLAnFSYCFApfJKMbiXNLwxj"
        );
    }
}
