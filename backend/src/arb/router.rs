...
pub struct RouterConfig {
    ...
    /// Whether backrunning is enabled
...
impl LiquidityGraph {
    ...
    pub fn upsert_pool(&mut self, mut pool: Pool) -> bool {
        ...
        if pool.dex == crate::pool::DexProtocol::UniswapV2
            && pool.id.to_lowercase().contains("aerodrome")
        {
            if pool.fee_bps != 1 {
                pool.fee_bps = 300;
            }
        ...
        let min_v2_reserve = {
            ...
            let thresh_a = if dec_a >= 2 {
                U256::from(10u64).pow(U256::from(dec_a - 2))
            } else {
                U256::from(1)
            };
            let thresh_b = if dec_b >= 2 {
                U256::from(10u64).pow(U256::from(dec_b - 2))
            } else {
                U256::from(1)
            };
        ...
        self.token_decimals
            .insert(pool_arc.token_a.address.clone(), pool_arc.token_a.decimals);
        self.token_decimals
            .insert(pool_arc.token_b.address.clone(), pool_arc.token_b.decimals);
        ...
        self.pools
            .insert(pool_arc.id.clone(), Arc::clone(&pool_arc));
        true
    }
    ...
    pub fn edges_from<'a>(
        &'a self,
        token: &'a str,
    ) -> impl Iterator<Item = &'a LiquidityEdge> + 'a {
    ...
    fn compute_rate_static(pool: &Pool, amount_in: U256, zero_for_one: bool) -> Option<f64> {
        ...
        let dec_in = if zero_for_one {
            pool.token_a.decimals
        } else {
            pool.token_b.decimals
        };
        let dec_out = if zero_for_one {
            pool.token_b.decimals
        } else {
            pool.token_a.decimals
        };
    ...
    pub fn find_opportunities(
        ...
        let first_hops: Vec<&LiquidityEdge> = self
            .edges
            .iter()
        ...
        for e1 in &first_hops {
            ...
            for e2 in self.edges.iter().filter(|e| {
                e.token_in == *mid && e.token_out == start_token && e.pool.last_updated_ts != 0
            }) {
                if e1.pool.id == e2.pool.id {
                    continue;
                }

                if let Some(mut arb) =
                    self.evaluate_two_hop(start_token, mid, &e1.pool, &e2.pool, config)
                {
                    arb.calculate_nev(
                        config.eth_price_usd,
                        config.btc_price_usd,
                        config.aave_fee_bps,
    ...
    fn evaluate_two_hop(
        ...
        use crate::arb::math::optimizer::{estimate_upper_bound, find_optimal_input};

        let zfo1 = start == pool1.token_a.address;
        let zfo2 = mid == pool2.token_a.address;

        let dec_start = if zfo1 {
            pool1.token_a.decimals
        } else {
            pool1.token_b.decimals
        };
        let dec_mid = if zfo1 {
            pool1.token_b.decimals
        } else {
        ...
        let upper_bound = if upper_bound > hard_cap_18 {
            hard_cap_18
        } else {
            upper_bound
        };
        ...
        let gas_cost_wei = U256::from(
            (config.gas_estimate as f64 * config.gas_price_gwei * 1_000_000_000.0) as u128,
        );
        let gas_cost_scaled = U256::from(
            (gas_cost_wei.low_u128() as f64 * config.eth_price_usd / start_price) as u128,
        );

        let optimal_amount_18 =
            match find_optimal_input(&sim_cycle_18, lower_bound, upper_bound, gas_cost_scaled) {
                Some(opt) => opt.amount,
                None => return None,
            };
        ...
        if imp1 > config.max_price_impact_bps {
            return None;
        }

        let out2 = sim_out(pool2, out1, zfo2)?;
        let imp2 = sim_impact(pool2, out1, zfo2);
        if imp2 > config.max_price_impact_bps {
            return None;
        }
...
fn bellman_ford_from(
    ...
    for _pass in 0..n.saturating_sub(1) {
        ...
        for (eidx, edge) in graph.edges.iter().enumerate() {
            if edge.pool.last_updated_ts == 0 {
                continue;
            } // Skip stale placeholder pools
            if edge.pool.fee_bps == 1 {
                continue;
            } // Skip Aerodrome V2 Stable pools (hallucinated arbs due to math mismatch)
    ...
    for &cycle_vertex in &neg_cycle_vertices {
        if let Some(opp) = reconstruct_and_evaluate(graph, config, &pred, cycle_vertex, seen_cycles)
        {
            opps.push(opp);
        }
    }
...
fn reconstruct_and_evaluate(
    ...
    let entry_decimals =
        if cycle_edges.first()?.token_in == cycle_edges.first()?.pool.token_a.address {
    ...
    let scaling_factor = if entry_decimals < 18 {
        U256::from(10u64)
            .checked_pow(U256::from((18 - entry_decimals) as u64))
            .unwrap_or(U256::one())
    } else {
        U256::one()
    };
    ...
    let get_edge_reserve = |e: &LiquidityEdge| -> (U256, u8) {
        ...
        let decimals = if zfo {
            e.pool.token_a.decimals
        } else {
            e.pool.token_b.decimals
        };
    ...
    let min_reserve = cycle_edges
        .iter()
        .map(|e| {
            let (reserve, decimals) = get_edge_reserve(e);
            let scaling = if decimals < 18 {
                U256::from(10u64)
                    .checked_pow(U256::from((18 - decimals) as u64))
                    .unwrap_or(U256::one())
            } else {
                U256::one()
            };
    ...
    let gas_cost_wei =
        U256::from((config.gas_estimate as f64 * config.gas_price_gwei * 1_000_000_000.0) as u128);
    let gas_cost_scaled =
        U256::from((gas_cost_wei.low_u128() as f64 * config.eth_price_usd / start_price) as u128);

    let sim_cycle = |amount_wei: U256| -> U256 {
        let mut amount = amount_wei
    ...
    let optimal_amount_wei =
        match find_optimal_input(&sim_cycle, lower_bound, upper_bound, gas_cost_scaled) {
            Some(opt) => opt.amount,
            None => {
    ...
    for edge in &cycle_edges {
        ...
        if impact > config.max_price_impact_bps {
            tracing::trace!(
                "BF cycle rejected: price impact {} bps > limit at optimal amount",
                impact
            );
            return None;
        }
    ...
    if opp.gross_return_bps() > 500 {
        tracing::trace!(
            "BF cycle rejected: infinite yield illusion ({} bps)",
            opp.gross_return_bps()
        );
        return None;
    }
...
mod tests {
    ...
    fn make_v2_pool(id: &str, tok_a: &str, tok_b: &str, res_a: u128, res_b: u128) -> Pool {
        Pool {
            ...
            token_a: Token {
                address: tok_a.to_string(),
                symbol: "A".to_string(),
                decimals: 18,
            },
            token_b: Token {
                address: tok_b.to_string(),
                symbol: "B".to_string(),
                decimals: 18,
            },
    ...
    fn test_two_hop_finds_profitable_arbitrage() {
        ...
        assert!(
            opps.len() <= 2,
            "Unexpected number of opportunities: {}",
            opps.len()
        );
    }
    ...
    fn test_bf_no_cycle_balanced_pools() {
        ...
        assert!(
            opps.is_empty() || opps.iter().all(|o| !o.is_executable),
    ...
    fn test_bf_detects_profitable_cycle() {
        ...
        for opp in &opps {
            assert!(
                !opp.route.is_empty(),
                "Opportunity should have at least one step"
            );
        }
    }
    ...
    fn test_cross_chain_fetch_specs_empty_for_evm_only_graph() {
        ...
        assert!(
            specs.is_empty(),
            "No non-EVM pools should produce fetch specs"
        );
    }
    ...
    fn test_cross_chain_fetch_specs_with_non_evm_pools() {
        ...
        let p_sol = make_v2_pool(
            "solana:sol_weth_usdc",
            "0xweth",
            "0xother",
            1_000_000_000_000_000_000_000_000,
            2_000_000_000_000_000_000_000_000,
        ...
        let p_osm = make_v2_pool(
            "cosmos:osm_usdc_atom",
            "0xusdc",
            "0xatom",
            1_000_000_000_000_000_000_000_000,
            2_000_000_000_000_000_000_000_000,
        ...
        let p_sol_non_overlap = make_v2_pool(
            "solana:sol_other",
            "0xother1",
            "0xother2",
            1_000_000_000_000_000_000_000_000,
        ...
        assert_eq!(
            specs.len(),
            2,
            "Should identify exactly two overlapping non-EVM pools"
        );

        let has_solana = specs
            .iter()
            .any(|s| s.chain == CrossChainTarget::Solana && s.pool_id == "solana:sol_weth_usdc");
        let has_osmosis = specs
            .iter()
            .any(|s| s.chain == CrossChainTarget::Osmosis && s.pool_id == "cosmos:osm_usdc_atom");
