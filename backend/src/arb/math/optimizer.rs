...
mod tests {
    ...
    fn test_golden_section_concave() {
        ...
        let diff = if opt.amount > expected_peak {
            opt.amount - expected_peak
        } else {
            expected_peak - opt.amount
        };
        assert!(
            diff < U256::from(2_000u64),
            "Should converge near peak K/2, diff = {:?}",
            diff
        );
    }
    ...
    fn test_golden_add_less_than_x2() {
        ...
        for diff in 1..100_000u64 {
            ...
            if next_x2 < x2 {
                panic!(
                    "Found violation! diff={}, x1={}, x2={}, next_x2={}",
                    diff, x1, x2, next_x2
                );
            }
        }
