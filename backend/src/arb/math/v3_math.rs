    ...
    if add {
        ...
        match amount.checked_mul(sqrt_price_x96) {
            Some(product) => {
                ...
                if denom >= numerator1 {
                    mul_div_rounding_up(numerator1, sqrt_price_x96, denom).unwrap_or(sqrt_price_x96)
                } else {
                    // Overflow branch
...
pub fn get_next_sqrt_price_from_amount1_rounding_down(
    ...
    let q96 = U256::from(Q96);
    if add {
        let quotient = mul_div(amount, q96, U256::from(liquidity)).unwrap_or(U256::zero());
        sqrt_price_x96 + quotient
    } else {
        let quotient =
            mul_div_rounding_up(amount, q96, U256::from(liquidity)).unwrap_or(U256::zero());
        if sqrt_price_x96 <= quotient {
            return U256::from(MIN_SQRT_RATIO);
        }
        sqrt_price_x96 - quotient
    }
}
...
pub fn compute_swap_step(
    ...
    if exact_in {
        ...
        let amount_remaining_less_fee =
            mul_div(amount_remaining, fee_complement, fee_denom).unwrap_or(U256::zero());
    ...
    let fee_amount = if sqrt_ratio_next_x96 != sqrt_ratio_target_x96 {
        ...
        mul_div_rounding_up(amount_in, U256::from(fee_pips), fee_complement).unwrap_or(U256::zero())
    };
...
mod tests {
    ...
    fn test_min_tick_gives_min_sqrt() {
        ...
        assert!(
            sqrt >= U256::from(MIN_SQRT_RATIO),
            "MIN_TICK sqrt too small: {:?}",
            sqrt
        );
    }
    ...
    fn test_compute_swap_step_basic() {
        ...
        let (next_sqrt, amt_in, amt_out, fee) =
            compute_swap_step(sqrt_price, target, liquidity, amount_in, 3000, true, true);

        assert!(
            next_sqrt < sqrt_price,
