...
const techStack = [
  {
    ...
    tech: "Rust + Tokio",
    note: "Max speed, memory safe",
  },
  {
    layer: "EVM Contracts",
    tech: "Solidity + Foundry",
    note: "Atomic arb + flashloans",
  },
  {
    layer: "Solana Contracts",
    tech: "Rust + Anchor",
    note: "High-freq Solana DEXs",
  },
  { layer: "Cosmos Logic", tech: "CosmWasm + Go", note: "IBC arbitrage" },
  {
    layer: "Frontend",
    tech: "Next.js 14 + Tailwind",
    note: "Crafted trading UI",
  },
  {
    layer: "Database",
    tech: "PostgreSQL + Redis",
    note: "Pool state + hot cache",
  },
  {
    layer: "Infra",
    tech: "AWS + Cloudflare",
    note: "Low-latency global nodes",
  },
];
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          />
          <span
            style={{ fontSize: 10, color: "#64748B", letterSpacing: "0.15em" }}
          >
            MEMPOOL STREAM
          </span>
        </div>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <span
            style={{
              ...
              textShadow: "0 0 5px rgba(0,255,209,0.3)",
            }}
          >
            ● LIVE
          </span>
      ...
      >
        <span title="Local system time the event was detected in the mempool stream">
          TIME
        </span>
        <span title="Type of network event (e.g., Block Sync, Pool Update, Swap)">
          EVENT TYPE
        </span>
        <span title="The unique transaction or block hash">TX HASH</span>
        <span title="The chain or rollup network (e.g., Base L2, Arbitrum)">
          NETWORK
        </span>
        <span title="The specific pool or token contract address targeted">
          TARGET
        </span>
        <span
          style={{ textAlign: "right" }}
          title="Transaction size or Gas Fee (Gwei) if applicable"
        >
          SIZE/GAS
        </span>
      </div>
      <div style={{ maxHeight: 180, overflowY: "hidden", padding: "4px 0" }}>
        {txs.map((tx, i) => {
          const timeStr = tx.ts
            ? new Date(tx.ts).toISOString().split("T")[1].slice(0, 12)
            : "00:00:00.000";
          return (
            <div
              key={`${tx.id}-${i}`}
            ...
            >
              ...
              <span
                style={{
                  color: tx.type === "BLOCK_SYNC" ? "#10B981" : "#94A3B8",
                  fontWeight: tx.type === "BLOCK_SYNC" ? 700 : 400,
                }}
                title={tx.hash}
              >
              ...
              <span
                style={{
                  color: tx.type === "POOL_SYNC" ? "#F59E0B" : "#E2E8F0",
                }}
              >
                {tx.token}
              </span>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          style={{ fontSize: 10, color: "#475569", fontFamily: "monospace" }}
        >
      ...
      {chainAdapters.map((adapter, i) => (
        <div
          ...
          style={{
            padding: "12px 16px",
            borderBottom:
              i < chainAdapters.length - 1 ? "1px solid #0D1117" : "none",
          }}
        >
          <div
            style={{
...
function IndexerStatsPanel() {
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        ...
        const data = await res.json();
        const hitRate =
          data.cache_hits + data.cache_misses === 0
            ? 0
  ...
  const metrics = [
    {
      label: "Pools Indexed",
      value: stats.poolsIndexed.toLocaleString(),
      unit: "",
      color: "#00FFD1",
    },
    {
      ...
      unit: "%",
      color: stats.cacheHitRate > 90 ? "#00FFD1" : "#FFD700",
    },
    {
      label: "Txs Seen",
      value: stats.indexLag.toLocaleString(),
      unit: "",
      color: "#00FFD1",
    },
    {
      label: "Opps Found",
      value: stats.redisKeys.toLocaleString(),
      unit: "",
      color: "#A78BFA",
    },
    {
      label: "Txs Decoded",
      value: stats.pgWrites.toLocaleString(),
      unit: "",
      color: "#64748B",
    },
  ];
        ...
        {metrics.map((m, i) => (
          <div
            ...
            style={{
              padding: "14px 12px",
              borderRight:
                i < metrics.length - 1 ? "1px solid #0D1117" : "none",
              textAlign: "center",
            }}
          >
            <div
            ...
            <div
              style={{ fontSize: 9, color: "#374151", letterSpacing: "0.08em" }}
            >
              {m.label}
            </div>
          </div>
    ...
    >
      ...
      >
        ...
        <span style={{ color: "#FF6B6B" }}>
          price_impact(pool_depth, trade_size)
        </span>
        <br />
        <br />
...
function BridgeLatencyTable({ color }) {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontSize: 10, color, letterSpacing: "0.2em" }}>
          BRIDGE LATENCY MATRIX
        </span>
      </div>
      <div
        style={{
          display: "grid",
          gridTemplateColumns: "1fr 100px 90px 120px 80px",
          gap: 0,
        }}
      >
        ...
        {rows.map((r, i) => [
          <div
            ...
            style={{
              ...
              fontSize: 10,
              color: "#94A3B8",
              borderBottom: i < rows.length - 1 ? "1px solid #0D1117" : "none",
            }}
          >
            {r.route}
          </div>,
      ...
      <div style={{ padding: "8px 4px" }}>
        {items.map((item, i) => (
          ...
          >
            ...
            />
            <span
              style={{ fontSize: 11, color: item.done ? "#CBD5E1" : "#475569" }}
            >
              {item.label}
            </span>
          </div>
            ...
            {techStack.map((t, i) => (
              <div
                ...
                style={{
                  ...
                  borderBottom:
                    i < techStack.length - 1 ? "1px solid #1A2233" : "none",
                  padding: "12px 20px",
              ...
              >
                ...
                <span
                  style={{ fontSize: 11, color: "#00FFD1", fontWeight: 600 }}
                >
                  {t.tech}
                </span>
      ...
      const next = prev.map((p, pi) => ({
        ...
        milestones: p.milestones.map((m, mi) => {
          ...
          const nextCompletion =
            nextStatus === "done" ? 100 : nextStatus === "in-progress" ? 50 : 0;
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          />
          <span
            style={{ fontSize: 10, color: "#64748B", letterSpacing: "0.25em" }}
          >
            ARB-DAPP
          </span>
        ...
        >
          ...
          {phaseData.map((p, i) => {
            const avgCompletion = Math.round(
              p.milestones.reduce((a, m) => a + m.completion, 0) /
                p.milestones.length,
            );
            return (
              <button
                ...
                style={{
                  ...
                  background:
                    active === i ? "rgba(255,255,255,0.03)" : "transparent",
                }}
              >
                <div
                  style={{
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: 9,
                      color: avgCompletion > 0 ? p.color : "#374151",
                      fontFamily: "monospace",
                    }}
                  >
                    {avgCompletion}%
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  <StatusBadge
                    status={
                      phase.milestones.every((m) => m.status === "done")
                        ? "done"
                  ...
                  style={{
                    ...
                    background:
                      m.status === "done"
                        ? phase.color
                        : m.status === "in-progress"
                          ? `linear-gradient(90deg, ${phase.color}, ${phase.color}00)`
                          : "transparent",
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  >
                    ...
                    <span
                      style={{
                        fontSize: 9,
                        color: m.completion > 0 ? phase.color : "#374151",
                        fontFamily: "monospace",
                      }}
                    >
                      {m.completion}%
