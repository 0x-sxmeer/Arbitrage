...
impl BundleBuilder {
    ...
    pub async fn submit_backrun_bundle(
        ...
        // Sign our backrun tx
        let signed_our_tx = self.sign_tx(
        ...
        info!(
            ...
            &bundle_hash[..16], target_block, opp.expected_profit
        );
    ...
    pub async fn submit_liquidation_tx(
        ...
        info!(
            ...
            &opp.borrower[..8], opp.net_profit_usd, &tx_hash[..16]
        );
    ...
    async fn submit_bundle(&self, bundle: FlashbotsBundle) -> Result<String> {
        ...
        if let Some(err) = body.error {
            return Err(anyhow::anyhow!("Flashbots error {}: {}", err.code, err.message));
        }
    ...
    fn sign_flashbots_header(&self, body: &str) -> String {
